...
use Tekstove\TekstoveBundle\Model\Entity\LyricRedirect;
use Tekstove\TekstoveBundle\Model\Entity\LyricRedirectQuery;
...
class LyricRedirectTableMap extends TableMap
{
     ...
     */
    const OM_CLASS = '\\Tekstove\\TekstoveBundle\\Model\\Entity\\LyricRedirect';
    ...
    public function initialize()
    {
        ...
        $this->addForeignPrimaryKey('redirect_id', 'RedirectId', 'INTEGER' , 'lyric', 'id', true, null, null);
    } // initialize()
  ...
  array (
     ...
     * and findPk*() calls.
     *
     * @param \Tekstove\TekstoveBundle\Model\Entity\LyricRedirect $obj A \Tekstove\TekstoveBundle\Model\Entity\LyricRedirect object.
    ...
    public static function addInstanceToPool($obj, $key = null)
    {
        if (Propel::isInstancePoolingEnabled()) {
            if (null === $key) {
                $key = serialize(array((string) $obj->getDeletedId(), (string) $obj->getRedirectId()));
            } // if key === null
     ...
     *
     * @param mixed $value A \Tekstove\TekstoveBundle\Model\Entity\LyricRedirect object or a primary key value.
     */
    public static function removeInstanceFromPool($value)
    {
        if (Propel::isInstancePoolingEnabled() && null !== $value) {
            if (is_object($value) && $value instanceof \Tekstove\TekstoveBundle\Model\Entity\LyricRedirect) {
                $key = serialize(array((string) $value->getDeletedId(), (string) $value->getRedirectId()));
                ...
                $e = new PropelException("Invalid value passed to removeInstanceFromPool().  Expected primary key or \Tekstove\TekstoveBundle\Model\Entity\LyricRedirect object; got " . (is_object($value) ? get_class($value) . ' object.' : var_export($value, true)));
                throw $e;
            }
    ...
    public static function getPrimaryKeyHashFromRow($row, $offset = 0, $indexType = TableMap::TYPE_NUM)
    {
        ...
        if ($row[TableMap::TYPE_NUM == $indexType ? 0 + $offset : static::translateFieldName('DeletedId', TableMap::TYPE_PHPNAME, $indexType)] === null && $row[TableMap::TYPE_NUM == $indexType ? 1 + $offset : static::translateFieldName('RedirectId', TableMap::TYPE_PHPNAME, $indexType)] === null) {
            return null;
        }

        return serialize(array((string) $row[TableMap::TYPE_NUM == $indexType ? 0 + $offset : static::translateFieldName('DeletedId', TableMap::TYPE_PHPNAME, $indexType)], (string) $row[TableMap::TYPE_NUM == $indexType ? 1 + $offset : static::translateFieldName('RedirectId', TableMap::TYPE_PHPNAME, $indexType)]));
    }
    ...
    public static function getPrimaryKeyFromRow($row, $offset = 0, $indexType = TableMap::TYPE_NUM)
    {
        ...
        $pks[] = (int) $row[
            $indexType == TableMap::TYPE_NUM
                ? 0 + $offset
                : self::translateFieldName('DeletedId', TableMap::TYPE_PHPNAME, $indexType)
        ];
        $pks[] = (int) $row[
            $indexType == TableMap::TYPE_NUM
                ? 1 + $offset
                : self::translateFieldName('RedirectId', TableMap::TYPE_PHPNAME, $indexType)
        ];
     ...
     */
     public static function doDelete($values, ConnectionInterface $con = null)
     {
        ...
        if ($values instanceof Criteria) {
            // rename for clarity
            $criteria = $values;
        } elseif ($values instanceof \Tekstove\TekstoveBundle\Model\Entity\LyricRedirect) { // it's a model object
            ...
            foreach ($values as $value) {
                $criterion = $criteria->getNewCriterion(LyricRedirectTableMap::COL_DELETED_ID, $value[0]);
                $criterion->addAnd($criteria->getNewCriterion(LyricRedirectTableMap::COL_REDIRECT_ID, $value[1]));
