...
use Tekstove\TekstoveBundle\Model\Lyric\LyricLanguage as ChildLyricLanguage;
 ...
 *
 * @method     \Tekstove\TekstoveBundle\Model\LyricQuery|\Tekstove\TekstoveBundle\Model\LanguageQuery endUse() Finalizes a secondary criteria and merges it with its primary Criteria
 *
...
abstract class LyricLanguageQuery extends ModelCriteria
{
    ...
    /**
     * Initializes internal state of \Tekstove\TekstoveBundle\Model\Lyric\Base\LyricLanguageQuery object.
     *
     * @param     string $dbName The database name
     * @param     string $modelName The phpName of a model, e.g. 'Book'
     * @param     string $modelAlias The alias for the model in this query, e.g. 'b'
     */
    public function __construct($dbName = 'default', $modelName = '\\Tekstove\\TekstoveBundle\\Model\\Lyric\\LyricLanguage', $modelAlias = null)
    {
    ...
    public function findPk($key, ConnectionInterface $con = null)
    {
        ...
        if ((null !== ($obj = LyricLanguageTableMap::getInstanceFromPool(serialize(array((string) $key[0], (string) $key[1]))))) && !$this->formatter) {
    ...
    protected function findPkSimple($key, ConnectionInterface $con)
    {
        ...
        if ($row = $stmt->fetch(\PDO::FETCH_NUM)) {
            ...
            $obj->hydrate($row);
            LyricLanguageTableMap::addInstanceToPool($obj, serialize(array((string) $key[0], (string) $key[1])));
        }
        $stmt->closeCursor();
     ...
     *
     * @param \Tekstove\TekstoveBundle\Model\Lyric|ObjectCollection $lyric The related object(s) to use as filter
     * @param string $comparison Operator to use for the column comparison, defaults to Criteria::EQUAL
     *
     * @throws \Propel\Runtime\Exception\PropelException
     *
    ...
    /**
     * Filter the query by a related \Tekstove\TekstoveBundle\Model\Language object
     *
     * @param \Tekstove\TekstoveBundle\Model\Language|ObjectCollection $language The related object(s) to use as filter
     * @param string $comparison Operator to use for the column comparison, defaults to Criteria::EQUAL
     *
     * @throws \Propel\Runtime\Exception\PropelException
     *
    ...
    {
        if ($language instanceof \Tekstove\TekstoveBundle\Model\Language) {
            return $this
            ...
            throw new PropelException('filterByLanguage() only accepts arguments of type \Tekstove\TekstoveBundle\Model\Language or Collection');
        }
    }
     ...
     * @param     string $joinType Accepted values are null, 'left join', 'right join', 'inner join'
     *
     * @return \Tekstove\TekstoveBundle\Model\LanguageQuery A secondary query class using the current class as primary query
     */
    public function useLanguageQuery($relationAlias = null, $joinType = Criteria::INNER_JOIN)
    {
        return $this
            ->joinLanguage($relationAlias, $joinType)
            ->useQuery($relationAlias ? $relationAlias : 'Language', '\Tekstove\TekstoveBundle\Model\LanguageQuery');
    }
