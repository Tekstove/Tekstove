...
use Tekstove\TekstoveBundle\Model\Acl\Map\PermissionGroupTableMap;
 ...
 *
 * @method     \Tekstove\TekstoveBundle\Model\Acl\PermissionGroupPermissionQuery|\Tekstove\TekstoveBundle\Model\Acl\PermissionGroupUserQuery endUse() Finalizes a secondary criteria and merges it with its primary Criteria
 *
...
abstract class PermissionGroupQuery extends ModelCriteria
{
    ...
    /**
     * Initializes internal state of \Tekstove\TekstoveBundle\Model\Acl\Base\PermissionGroupQuery object.
     *
     * @param     string $dbName The database name
     * @param     string $modelName The phpName of a model, e.g. 'Book'
     * @param     string $modelAlias The alias for the model in this query, e.g. 'b'
     */
    public function __construct($dbName = 'default', $modelName = '\\Tekstove\\TekstoveBundle\\Model\\Acl\\PermissionGroup', $modelAlias = null)
    {
    ...
    public function findPk($key, ConnectionInterface $con = null)
    {
        ...
        if ((null !== ($obj = PermissionGroupTableMap::getInstanceFromPool((string) $key))) && !$this->formatter) {
    ...
    protected function findPkSimple($key, ConnectionInterface $con)
    {
        ...
        if ($row = $stmt->fetch(\PDO::FETCH_NUM)) {
            ...
            $obj->hydrate($row);
            PermissionGroupTableMap::addInstanceToPool($obj, (string) $key);
        }
        $stmt->closeCursor();
    ...
    /**
     * Filter the query by a related \Tekstove\TekstoveBundle\Model\Acl\PermissionGroupPermission object
     *
     * @param \Tekstove\TekstoveBundle\Model\Acl\PermissionGroupPermission|ObjectCollection $permissionGroupPermission the related object to use as filter
     * @param string $comparison Operator to use for the column comparison, defaults to Criteria::EQUAL
     *
    ...
    {
        if ($permissionGroupPermission instanceof \Tekstove\TekstoveBundle\Model\Acl\PermissionGroupPermission) {
            return $this
            ...
            throw new PropelException('filterByPermissionGroupPermission() only accepts arguments of type \Tekstove\TekstoveBundle\Model\Acl\PermissionGroupPermission or Collection');
        }
    }
     ...
     * @param     string $joinType Accepted values are null, 'left join', 'right join', 'inner join'
     *
     * @return \Tekstove\TekstoveBundle\Model\Acl\PermissionGroupPermissionQuery A secondary query class using the current class as primary query
     */
    public function usePermissionGroupPermissionQuery($relationAlias = null, $joinType = Criteria::INNER_JOIN)
    {
        return $this
            ...
            ->useQuery($relationAlias ? $relationAlias : 'PermissionGroupPermission', '\Tekstove\TekstoveBundle\Model\Acl\PermissionGroupPermissionQuery');
    }
