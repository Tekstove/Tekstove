...
abstract class PermissionGroupPermissionQuery extends ModelCriteria
{
    ...
    public function findPk($key, ConnectionInterface $con = null)
    {
        ...
        if ((null !== ($obj = PermissionGroupPermissionTableMap::getInstanceFromPool(serialize(array((string) $key[0], (string) $key[1]))))) && !$this->formatter) {
    ...
    protected function findPkSimple($key, ConnectionInterface $con)
    {
        ...
        if ($row = $stmt->fetch(\PDO::FETCH_NUM)) {
            ...
            $obj->hydrate($row);
            PermissionGroupPermissionTableMap::addInstanceToPool($obj, serialize(array((string) $key[0], (string) $key[1])));
        }
        $stmt->closeCursor();
    ...
    /**
     * Filter the query by a related \Tekstove\TekstoveBundle\Model\Acl\Permission object
     *
     * @param \Tekstove\TekstoveBundle\Model\Acl\Permission|ObjectCollection $permission The related object(s) to use as filter
     * @param string $comparison Operator to use for the column comparison, defaults to Criteria::EQUAL
     *
     * @throws \Propel\Runtime\Exception\PropelException
     *
    ...
    {
        if ($permission instanceof \Tekstove\TekstoveBundle\Model\Acl\Permission) {
            return $this
            ...
            throw new PropelException('filterByPermission() only accepts arguments of type \Tekstove\TekstoveBundle\Model\Acl\Permission or Collection');
        }
    }
     ...
     * @param     string $joinType Accepted values are null, 'left join', 'right join', 'inner join'
     *
     * @return \Tekstove\TekstoveBundle\Model\Acl\PermissionQuery A secondary query class using the current class as primary query
     */
    public function usePermissionQuery($relationAlias = null, $joinType = Criteria::INNER_JOIN)
    {
        return $this
            ->joinPermission($relationAlias, $joinType)
            ->useQuery($relationAlias ? $relationAlias : 'Permission', '\Tekstove\TekstoveBundle\Model\Acl\PermissionQuery');
    }
